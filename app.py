...
class Books:
    def __init__(self):
        self.books = pd.read_csv('./Datasets/Books.csv')
...
def predict():
    ...
    if request.method == 'POST':
        ...
        KNN_Recommended_Books = KNN_Recommended_Books.rename(
            columns={'bookRating': 'MeanRating'})
        # print(KNN_Recommended_Books)
        bookTitle = KNN_Recommended_Books["bookTitle"].tolist()
        bookAuthor = KNN_Recommended_Books["bookAuthor"].tolist()
        ISBN = KNN_Recommended_Books["ISBN"].tolist()
        publisher = KNN_Recommended_Books["publisher"].tolist()
        MeanRating = KNN_Recommended_Books["MeanRating"].tolist()
        books=[]
        books.append([bookTitle,bookAuthor,ISBN,publisher,MeanRating])
        # print(books[0])
